...
class CartAndItemModelsTestCase(TestCase):
    ...
    def test_item_creation_and_association_with_cart(self):
        ...
        item = self._create_item_in_database(cart, user, quantity=1,
                                             unit_price=Decimal("100"))

        # get the first item in the cart
        item_in_cart = cart.item_set.all()[0]
        self.assertEquals(item_in_cart, item,
                          "First item in cart should be equal the item"
                          " we created")
        self.assertEquals(item_in_cart.product, user,
                          "Product associated with the first item in cart "
                          "should equal the user we're selling")
        self.assertEquals(item_in_cart.unit_price, Decimal("100"),
                          "Unit price of the first item stored in the cart "
                          "should equal 100")
        self.assertEquals(item_in_cart.quantity, 1,
                          "The first item in cart should have 1 in "
                          "it's quantity")
    ...
    def test_total_item_price(self):
        ...
        # not safe to do as the field is Decimal type.
        # It works for integers but doesn't work for float
        item_with_unit_price_as_integer = self._create_item_in_database(
            cart, product=user, quantity=3, unit_price=100
        )
        ...
        self.assertEquals(item_with_unit_price_as_decimal.total_price,
                          Decimal("12.80"))
    ...
    def test_item_unicode(self):
        ...
        item = self._create_item_in_database(cart, product=user, quantity=3,
                                             unit_price=Decimal("100"))

        self.assertEquals(force_text(item), "3 units of User")
